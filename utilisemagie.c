...
void boobooble_power(SDL_Surface* ecran, struct Stat* monstre2, int mobselectionne, int degatmin, int degatmax)
{
    int x;
    int nombreMystere;
    ...
    {
        nombreMystere = (rand() % (AttMagMax - AttPhyMin + degatmax - degatmin) + AttPhyMin + degatmin) + 1 - monstre2->DefMag;
        afficheVie(ecran, 100);
...
void attaquemagique(SDL_Surface* ecran, int *x, int *continuer, int *fairetantque, int manaaenleve, int degatmin, int degatmax)
{
    ...
    while (laisse == 1 & *x != 0)
    {
        ...
        if (tempsActuel - tempsPrecedent > 30)     
        {
            ...
            texte = IMG_Load("images/combat/curseur2.bmp");
            SDL_SetColorKey(texte, SDL_SRCCOLORKEY, SDL_MapRGB(texte->format, 110, 255, 48));
            SDL_BlitSurface(texte, NULL, ecran, &position);
            SDL_FreeSurface(texte);
            
            if (renvoievaleur & manarestant >= manaaenleve)
            {
                if (letrait == 47)
                    boobooble_power(ecran, &monstre1, mobselectionne, degatmin, degatmax);
                if (letrait == 94)
                    boobooble_power(ecran, &monstre2, mobselectionne, degatmin, degatmax);
                if (letrait == 141)
                    boobooble_power(ecran, &monstre3, mobselectionne, degatmin, degatmax);
                *fairetantque = 2;
                laisse = 0;
                manarestant -= manaaenleve;
            }
...
void utilisemagie(SDL_Surface* ecran, int *choice, int *x, int *continuer, int *fairetantque)
{
    ...
    texte = TTF_RenderText_Blended(police3, "Quel sort voulez-vous lancer?", couleurBlanche);
    position.x = tailleecranL / 4 + 80;
    position.y = 7;
    SDL_BlitSurface(texte, NULL, ecran, &position);
    SDL_FreeSurface(texte);
        ...
        while (*choice == 2 & *x != 0 & maisse == 1)
        {
            ...
            if (tempsActuel - tempsPrecedent > 30)     
            {
                ...
                if (renvoievaleur == 2)
                {
                    for ( i = 0; i <= 9; i ++)
                    {
                        if (magies[i].posi == letrait + 25)
                        {
                            switch (i)
                            {
                                case 0:
                                    attaquemagique(ecran, x, continuer, &maisse, 3, 1, 5);
                                    if (maisse == 2)
                                    {
                                        *choice = 0;
                                        *fairetantque = 0;
                                ...
                                case 1:
                                    attaquemagique(ecran, x, continuer, &maisse, 7, 10, 15);
                                    if (maisse == 2)
                                    {
                                        *choice = 0;
                                        *fairetantque = 0;
    ...
    {
        texte = TTF_RenderText_Blended(police3, "Vous n'avez aucun sort !", couleurBlanche);
